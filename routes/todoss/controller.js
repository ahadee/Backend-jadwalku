...
module.exports = {
    ...
    getAllTodo: async (req, res) => {
        try {
            ...
            DetailTodos.belongsTo(Todos, { foreignKey: "id_list_todo" });
            Todos.hasOne(DetailTodos, { foreignKey: "id" });
            const {id, createdAt} = req.params
            const result = await DetailTodos.findAll({
                where: {
                    userID: id,
                    // createdAt: {
                    //     [Op.eq]: '2020-06-10',
            ...
            res.status(200).json({
                message: "Get all todos data for users",
                data: result,
            });
        } catch (error) {
            console.log(error);
